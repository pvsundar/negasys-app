...
Streamlit Web Application - Version 5.0
...
st.set_page_config(
    page_title="NEGASYS v5.0: Household Product Line Optimizer",
    page_icon="👫",
    layout="wide",
...
def main():
    st.title("👫 NEGASYS v5.0")
    st.markdown("**Negotiated Household Product Line Design Optimizer**")
    st.markdown("*With Real Data Support, Stage 0 Integration, and Multi-Rule Comparison*")
    ...
    with st.sidebar:
        ...
        data_source = st.radio(
            "Select Data Source",
            ["Synthetic Data", "Real Data (TV Format)"],
            help="Synthetic: generate random households. Real Data: upload .01b and .01c files."
        )
    ...
    with col1:
        if data_source == "Synthetic Data":
            if st.button("🔄 Generate Synthetic Data + Run Stage 0", 
            type="primary", 
            width="stretch"
            ):
            ...
            if utility_file is not None and competitor_file is not None:
                if st.button("📥 Load Real Data + Run Stage 0", 
                type="primary", 
                width="stretch"
                ):
                    with st.spinner("Loading and processing real data..."):
                        try:
                            ...
                            st.session_state.data_metadata['source'] = 'real'
    ...
    if st.session_state.household_data is not None:
        ...
        with col1:
            st.metric("Source", metadata.get('source', 'unknown').title())
        with col2:
        ...
        with st.expander("📋 Attribute Details"):
            ...
            st.dataframe(pd.DataFrame(attr_data), hide_index=True, width="stretch")
        ...
        **Configuration:**
        ...
        with col1:
            run_single = st.button("🚀 Run Single Optimization", type="primary", width="stretch")
        
        with col2:
            run_all = st.button("🔬 Test All 4 Decision Rules", 
        ...
        if st.session_state.result:
            ...
            st.dataframe(pd.DataFrame(products_data), hide_index=True, width="stretch")
        ...
        st.info("👈 Generate or load data using Step 1 to begin.")
...
def show_footer():
    ...
    st.markdown(f"""
    ---
    **NEGASYS v{NEGASYS_VERSION}** | December 2025 | P.V. (Sundar) Balakrishnan
